...
<script>
  document.addEventListener("DOMContentLoaded", function(e) {
    var elements = document.getElementsByClassName("amazingcarousel-item-container");
    console.log(elements);
    for (var i = 0; i < elements.length; i++) {
      for (var j = 0; j < elements[i].children.length; j++) {
        if(elements[i].children[j].className == "amazingcarousel-title"){
          var nombreClase = elements[i].children[j].children[0].classList[0];
        }
      };
      elements[i].className += " "+nombreClase;
    }
  })
  window.addEventListener('scroll', function () {
    if (window.scrollY > 80) {
      document.querySelector("#site-banner .site-logo img").style.height = "62px";
      document.querySelector("#site-banner .site-logo img").style.transition = "all .5s";
      document.querySelector(".slope-header").style.height = "20px";
      document.querySelector(".slope-header").style.transition = "all .5s";
    }else{
      document.querySelector("#site-banner .site-logo img").style.height = "125px";
      document.querySelector("#site-banner .site-logo img").style.transition = "all .5s";
      document.querySelector(".slope-header").style.height = "41px";
      document.querySelector(".slope-header").style.transition = "all .5s";
    }
